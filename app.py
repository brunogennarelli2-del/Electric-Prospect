...
# -------------------------
# Filters (including new email/phone filters and presence)
# -------------------------
...
has_contact = st.sidebar.selectbox(
    "Has contact", ["All", "Has email", "Has phone", "Has both", "Has neither"], index=0,
)
preferred_contact = st.sidebar.radio(
    "Preferred contact", ["Auto", "Email", "Phone"], horizontal=True,
)
...
if query:
    ...
    f = f[mask]
if email_contains:
    f = f[f["Email"].str.contains(email_contains, case=False, na=False)]
if phone_contains:
    f = f[f["Phone"].astype(str).str.contains(phone_contains, case=False, na=False)]

has_email = f["Email"].str.contains("@", na=False)
has_phone = f["Phone"].astype(str).str.strip().ne("")
if has_contact == "Has email":
    f = f[has_email]
elif has_contact == "Has phone":
    f = f[has_phone]
elif has_contact == "Has both":
    f = f[has_email & has_phone]
elif has_contact == "Has neither":
    f = f[~has_email & ~has_phone]
...
with tab_overview:
    ...
    tbl["Contact"] = [render_contact(e, p) for e, p in zip(tbl["Email"], tbl["Phone"])]

    visible_cols = st.multiselect(
        "Columns to display",
        ["Name","Email","Phone","Contact","Company","Role","Country","Region","Status","Priority","Owner","LastContacted","NextFollowUp","PresentInCRM","Score","Notes"],
        default=["Name","Contact","Company","Role","Country","Region","Status","Priority","Owner","PresentInCRM","Score"],
    )
    ...
    if dfc.empty:
        ...
        for _, row in dfc.sort_values(["Score"], ascending=False).head(60).iterrows():
            ...
            if isinstance(row["Email"], str) and "@" in row["Email"]:
                items.append(f"📧 <a href=\"mailto:{row['Email']}\">{row['Email']}</a>")
